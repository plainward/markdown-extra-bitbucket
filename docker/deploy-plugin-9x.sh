#!/bin/bash
# Deploy MarkdownX plugin to Bitbucket 9.x DC (Docker) via UPM REST API
#
# Usage: ./deploy-plugin-9x.sh [USERNAME] [PASSWORD]
#
# Expects JAR in ../target/

BITBUCKET_URL="http://localhost:7992"
USERNAME="${1:-admin}"
PASSWORD="${2:-admin}"
...
echo "Deploying $PLUGIN_JAR to $BITBUCKET_URL (Bitbucket 9.x)..."
