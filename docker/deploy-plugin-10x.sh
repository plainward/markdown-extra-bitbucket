#!/bin/bash
# Deploy MarkdownX plugin to Bitbucket 10.x DC (Docker) via UPM REST API
#
# Usage: ./deploy-plugin-10x.sh [USERNAME] [PASSWORD]
#
# Expects JAR in ../target/

BITBUCKET_URL="http://localhost:7991"
USERNAME="${1:-admin}"
PASSWORD="${2:-admin}"

PLUGIN_JAR=$(ls -t ../target/markdown-extra-*-bb10.jar 2>/dev/null | head -1)
...
echo "Deploying $PLUGIN_JAR to $BITBUCKET_URL (Bitbucket 10.x)..."
